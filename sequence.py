...
# import argparse
# import sys
# sys.path.append('/Users/PsychoMugs/projects/honda/src')
# import matplotlib
# matplotlib.use('agg')
# import importlib
# # import data
...
# import keras.losses
# from networks import kl_loss
# keras.losses.kl_loss = kl_loss
# from blossom import b_loss, tower_ran, base_ran, motor_ran
# keras.losses._b_loss = b_loss()
# model = load_model('/Users/PsychoMugs/projects/honda/results/ae_blossom/275/models/aeclf_blossom.h5')
# from scipy.signal import savgol_filter
# input_shape = model.input.shape.as_list()
# input_shape[0] = 1
# seq_chunk_len = input_shape[1]

# def scale(x, ran=[-1,1], scale_ran=[-1,1]):
#     if (len(np.array(ran).shape) == 1) and len(x.shape)>1:
#         ran = np.tile(ran,(x.shape[1],1))
#     if (len(np.array(scale_ran).shape) == 1) and len(x.shape)>1:
#         scale_ran = np.tile(scale_ran,(x.shape[1],1))
#     return np.transpose([np.interp(d,r,sr) for d,r,sr in zip(np.transpose(x),ran,scale_ran)])

# def classify(seq):
#     seq_list = np.transpose(seq.to_list()[2])
#     print(seq_list)
#     seq_list = scale(seq_list*np.pi/180, ran=motor_ran, scale_ran=[-1,1])
#     seq_list = seq_list[:,0:4]
#     # print(seq_list)
#     # chunk
#     seq_chunk = []
#     start_idx = 0

#     while start_idx<len(seq_list):
#         _seq_chunk = seq_list[start_idx:min(start_idx+seq_chunk_len, len(seq_list)),:]
#         print(_seq_chunk)
#         if len(_seq_chunk)<seq_chunk_len:
#             # break
#             append_chunk = np.tile(_seq_chunk[-1,:],[seq_chunk_len-len(_seq_chunk),1])
#             _seq_chunk = np.concatenate([_seq_chunk, append_chunk],axis=0)
#         # np.pad(_seq_chunk, pad_width=[[0,seq_chunk_len-len(_seq_chunk)],[0, mode='constant', constant_values=_seq_chunk[-1,:])

#         # seq_chunk.append(seq_list[start_idx:min(start_idx+)])
#         seq_chunk.append(_seq_chunk)
#         start_idx += seq_chunk_len//2
#     print(seq_chunk)
#     print([s.shape for s in seq_chunk])

#     print(model.predict(np.array(seq_chunk))[2])
...
class Sequence():
    ...
    @classmethod
    def from_dict(cls, millis_list, pos_list, seq_name=''):
        frames = []
        for i,(m,p) in enumerate(zip(millis_list, pos_list)):
            frames.append(Frame(m,p))
        return cls(seq_name, frames)
    ...
    def get_frame_diff(self, start_idx, end_idx):
        """
        Get the difference between 2 frames
        args:
            start_idx   first frame
            end_idx     second frame
        """
        return self.frames[end_idx].to_np()-self.frames[start_idx].to_np()


    def trim(self, trim_mode=0, threshold=1.0):
        """
        "Whitespace trimming" to remove static pose
        args:
            trim_mode   0=end, 1=start, 2=both
            threshold   difference to count a frame as new
        """ 
        last_idx = len(self.frames)-1
        end_idx = len(self.frames)-1
        while end_idx > 0 and np.max(np.abs(self.get_frame_diff(end_idx, last_idx))) < threshold:
            end_idx -= 1
        self.frames = self.frames[:end_idx+1]
        return self.frames
    ...
    def to_file(self,seq_name='',robot_dir='./',force=False):
        ...
        # save to file
        json_dict = {"animation": seq_name, "frame_list": frames_list}
        with open(robot_dir + seq_name + '_sequence.json', 'w') as seq_file:
            json.dump(json_dict, seq_file, indent=2)
        try:
            with open(robot_dir + seq_name + '_sequence.json', 'w') as seq_file:
                json.dump(json_dict, seq_file, indent=2)
        except TypeError:
            with open(robot_dir + seq_name + '_sequence.json', 'w') as seq_file:
                json.dump(str(json_dict).replace('\n','').replace("'",'"'), seq_file, indent=2)
        ...
        for f in frames:
            ...
            for m,p in list(goal_pos.items()):
                self.robot.goto_position(
                    {m:p},
                    duration=t_delay*(1.0/vel_pos[m]),
                    # duration=0.1,
                    wait=False)
...
class RecorderPrimitive(pypot.primitive.Primitive):
    ...
    def save_rec(self, seq_name, robots=[], tmp=False):
        ...
        for robot in robots:
            if not tmp:
                # truncate frames list
                seq = Sequence(seq_name, self.frames)
                self.frames_list = self.frames_list[:len(seq.trim())]
                # classify(seq)
                robot.add_sequence(seq)
